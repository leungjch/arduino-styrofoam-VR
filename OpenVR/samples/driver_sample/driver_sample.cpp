...
public:
	...
	virtual DriverPose_t GetPose() 
	{
		...
		if ((GetAsyncKeyState(VK_NUMPAD3) & 0x8000) != 0) yaw += 0.1;
		if ((GetAsyncKeyState(VK_NUMPAD1) & 0x8000) != 0) yaw += -0.1;

		if ((GetAsyncKeyState(VK_NUMPAD4) & 0x8000) != 0) pitch += 1;
		if ((GetAsyncKeyState(VK_NUMPAD6) & 0x8000) != 0) pitch += -1;

		if ((GetAsyncKeyState(VK_NUMPAD8) & 0x8000) != 0) roll += 1;
		if ((GetAsyncKeyState(VK_NUMPAD2) & 0x8000) != 0) roll += -1;
		...
		if ((GetAsyncKeyState(VK_UP) & 0x8000) != 0) pZ += -0.05;
		if ((GetAsyncKeyState(VK_DOWN) & 0x8000) != 0) pZ += 0.05;

		if ((GetAsyncKeyState(VK_LEFT) & 0x8000) != 0) pX += -0.05;
		if ((GetAsyncKeyState(VK_RIGHT) & 0x8000) != 0) pX += 0.05;
		
		if ((GetAsyncKeyState(VK_PRIOR) & 0x8000) != 0) pY += 0.05;
		if ((GetAsyncKeyState(VK_NEXT) & 0x8000) != 0) pY += -0.05;
		...
		if (!arduino.isInit)
		{
			...
			if (arduino.isConnected())
			{
				...
				//MessageBoxA(NULL, "connection established", "arduino", MB_OK);
				arduino.isInit = true;
				//Sleep(5000);
			}
			else
			{
				//MessageBoxA(NULL, "ERROR, check port name", "arduino", MB_OK);
			}
		}

		// read from arduino sensor
		//MessageBoxA(NULL, std::to_string(arduino.isConnected()).c_str(), "arduinojsconnnect", MB_OK);
		if (arduino.isConnected()) {
			//Check if data has been read or not
			int read_result = arduino.readSerialPort(incomingData, MAX_DATA_LENGTH);

			std::string data = std::string(incomingData);
			
			std::size_t i = data.find("GX") + 2;
			double gyroX = std::stod(data.substr(i, 8));
			roll -= gyroX;

			i = data.find("GY") + 2;
			double gyroY = std::stod(data.substr(i, 8));
			pitch -= gyroY;

			i = data.find("GZ") + 2;
			double gyroZ = std::stod(data.substr(i, 8));
			yaw -= gyroZ;
			//MessageBoxA(NULL, std::to_string(gyroZ).c_str(), "arduino", MB_OK);
			//prints out data
			//puts(incomingData);
			//wait a bit
			Sleep(6);
